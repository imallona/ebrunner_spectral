...
FEATURES <- file.path('/home/imallona', 'giulia',
                      'whole_genome')
```
...
# Covered vs true transcripts, stratified by number of probes per gene {.tabset .tabset-pills}

```{r, results = 'asis', fig.width = 7, fig.height = 7}

fd <- fd[order(fd$score, decreasing = TRUE),]
...
for (num_probes in 1:10) {
    cat(sprintf('## Probes per gene: %s {.tabset .tabset-pills}\n\n', num_probes))
    reduced <- Reduce(rbind.data.frame, by(fd, fd['gene_id'], head, n = num_probes))

    
    ## to get the num transcripts covered per gene, we need to aggregate
    ## the unique transcripts per gene from the probes'data

    covered <- with(reduced, tapply(transcripts, list(gene_id),
                                    FUN = function(x)
    ...
    missing <- data.frame(gene_id = unique(gtf$gene_id)[!unique(gtf$gene_id) %in%
                                                        unique(reduced$gene_id)],
                          num_covered_tx = 0)
    ...
    cat('### Genes status probes:', num_probes, '\n\n')
    
    cat(sprintf('Gene status with %s probes per gene\n\n', num_probes))
    print(knitr::kable(as.data.frame(table(tmp$status)), format = "html"))

    cat('\n\n')
    
    cat('### Plot probes:', num_probes, '\n\n')
    
    par(mfrow = c(2,2), pty = "s", mai = c(0.6, 0.6, 0.6, 0.2))
    ...
    mtext(sprintf("%s probes per gene", num_probes),
