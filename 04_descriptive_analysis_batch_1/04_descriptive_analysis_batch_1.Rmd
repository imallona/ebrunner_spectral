...
alien_counts <- tmp_counts[rownames(tmp_counts) %in% c('TDTOMATO', 'EGFP'),]
tmp_counts <- tmp_counts[!rownames(tmp_counts) %in% c('TDTOMATO', 'EGFP'),]
rownames(alien_counts) <- tolower(rownames(alien_counts))
...
altExp(sce, "alien") <- SummarizedExperiment(assays = list(counts = alien_counts))
...
```{r binarize_species, fig.height = 3, fig.width = 6}
THRES <- 0.7
par(mfrow = c(1,2))
...
# Diff expression (by unsupervised, integrated cluster) {.tabset .tabset-fade .tabset-pills}


```{r}
Idents(so) <- 'integrated_snn_res.0.1'
...
Idents(so) <- 'integrated_snn_res.0.1'
DoHeatmap(so, features = top10$gene) + ggtitle('by unsup. cluster')
```

## Same top 10 but plotting by mod/unmod and species

```{r, fig.width = 8, fig.height = 8}
Idents(so) <- 'compound_id'
DoHeatmap(so, features = top10$gene) + ggtitle('by species and name')
```

## Cross-tabulation

```{r}
table(so@meta.data$compound_id,
      so@meta.data$integrated_snn_res.0.1)
```
...
saveRDS(file = sprintf('%s_so.rds', ID), object = so)
```
